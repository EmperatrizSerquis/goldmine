...
class Store_items extends Trongate {
    ...
    function _get_item_pic_html($data) {
        ...
        $data['gallery_pics'] = $this->picture_uploader_multi->_fetch_pictures($update_id, $multi_uploader_settings);

        if (count($data['gallery_pics'])>1) {
            $data['gallery_dir'] = BASE_URL.$multi_uploader_settings['destination'].'/'.$update_id.'/';
            $item_pic_html = $this->view('item_gallery', $data, true);
        } else {
            //single picture
            if ($data['item_obj']->picture == '') {
                ...
                $data['picture_path'] = BASE_URL.$single_picture_settings['destination'].'/'.$update_id.'/'.$picture;
        ...
        if ($submit == 'Submit') {
            ...
            if ($result == true) {
                ...
                if (is_numeric($update_id)) {
                    ...
                    $data['item_code'] = make_rand_str(6, true); //generate random code
