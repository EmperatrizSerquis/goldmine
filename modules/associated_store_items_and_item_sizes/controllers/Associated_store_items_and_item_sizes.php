<?php
class Associated_store_items_and_item_sizes extends Trongate {

    private $associated_modules = ['store_items', 'item_sizes'];
    private $assoc_module = 'associated_store_items_and_item_sizes';
    ...
    function _draw_association_info($token) {
        ...
        if ($data['calling_module'] == 'store_items') {
            $data['associated_records_singular'] = 'Item Size ';
            $data['associated_records_plural'] = 'item sizes';
        } else {
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_store_items_and_item_sizes.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_store_items_and_item_sizes ON associated_module_name.id = associated_store_items_and_item_sizes.associated_module_name_id
                WHERE
                    associated_store_items_and_item_sizes.calling_module_name_id = '.$update_id;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_store_items_and_item_sizes                ON '.$associated_module_name.'.id = associated_store_items_and_item_sizes.'.$associated_module_name.'_id 
                WHERE associated_store_items_and_item_sizes.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_store_items_and_item_sizes.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_store_items_and_item_sizes.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_store_items_and_item_sizes.'.$calling_module_name.'_id IS NULL';
    ...
    function _get_associated_module_name($calling_module_name) {
        ...
        if ($calling_module_name == 'store_items') {
            $associated_module_name = 'item_sizes';
        } else {
    ...
    function _prep_inbound($input) {
        ...
        if ($params['callingModule'] == 'item_sizes') {
            $data['store_items_id'] = $params['selectedValue'];
            $data['item_sizes_id'] = $params['update_id'];
        } else {
            $data['item_sizes_id'] = $params['selectedValue'];
